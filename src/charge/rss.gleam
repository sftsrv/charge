...
pub fn with_rss(
  ...
  charge.with_summary(pipeline, fn(assets, agg) {
    let items =
      list.map(assets, fn(asset) {
        use html <- charge.if_html(asset, None)
        get_item(agg, html)
      })
      |> option.values
...
fn atom_link(base_url) {
  xml_element(
    "atom:link",
    [
      attr.href(base_url <> "/feed/rss.xml"),
      attr.rel("self"),
