...
fn render_rec(
  data,
  components: Dict(String, Visit(ChargeResult(ElementTree))),
  el: ElementTree,
...
pub fn render(
  ...
  out_dir: fs.Path,
  html: ElementTree,
  components: List(Component(ChargeResult(ElementTree))),
) {
  components
  |> dict.from_list
...
fn render_rec_async(
  data,
  tag,
  visit: fn(RenderData, ElementTree) -> Promise(ChargeResult(ElementTree)),
  el: ElementTree,
...
pub fn render_async(
  ...
  out_dir: fs.Path,
  html: ElementTree,
  component: Component(Promise(Result(ElementTree, error.ChargeError))),
) -> Promise(ChargeResult(ElementTree)) {
